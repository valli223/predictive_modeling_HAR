---
title: "Prediction of Human Activity Recognition"
...
dt <- data.frame(Dataset = c("training", "validation", "testing"), 
...
The results of the above analysis is summarized to find the model having the best accuracy. We observe that **Boosting Tree model with Bootstrap Resampling Model** has the highest accuracy and so we choose it.
...
In this section, we use the model that we selected in the last section to predict the test data. After comparing with the correct results, we observe that our model gives **`r round(p3[1] * 100, 2)`** Accuracy.
